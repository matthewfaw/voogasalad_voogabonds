package engine.model.systems;

import java.util.List;
import engine.IObserver;
import engine.model.ICollidable;


/**
 * All objects must be subscribed as ICollidable
 * NOT IObservables
 *
 */
public class CollisionDetectionTemp implements IObserver<ICollidable>{
	
	List<ICollidable> myObservables;
	
	public CollisionDetectionTemp() {
		//TODO: get list of all CollisionObservable objects on map
	}

	/**
	 * Determine if observable a intersects with observable b
	 * @param a
	 * @param b
	 * @return
	 */
	private boolean intersects(ICollidable a, ICollidable b) {
		double a_x = a.getLocation().getX();
		double a_y = a.getLocation().getY();
		double a_r = a.getRadius();
		
		double b_x = b.getLocation().getX();
		double b_y = b.getLocation().getY();
		double b_r = b.getRadius();
		
		return Math.pow(a_r - b_r, 2) <= 
				Math.pow(a_x - b_x, 2) + 
				Math.pow(a_y - b_y, 2);
	}

	/**
	 * On collision, this system will be notified and
	 * this method will be called.
	 */
	@Override
	public void update(ICollidable movedObservable) {
		for(ICollidable observable: myObservables) {
			if (intersects(movedObservable, observable)) {
				// notify collision detection strategy
			}
		}
	}

}
