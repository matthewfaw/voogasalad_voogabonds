...
public class MapDisplay implements IObserver<TimelineController>, IGUIPiece {
	...
	// public void giveViewableComponent(IObservable<IViewablePhysical>
	// aObservable)
	// {
	...
	public void setMap(MapDataContainer aMapData) {
		myBackground = new MapGrid(aMapData.getNumXCells(), aMapData.getNumYCells(), aMapData.getCellSize(),
				myAppController);
		for (TerrainData terrainData : aMapData.getTerrainList()) {
			...
			myPane.getChildren().add(myBackground.fillCell((int) terrainData.getLoc().getX(),
					(int) terrainData.getLoc().getY(), terrainData.getSize(), terrainData.getColor(), myRoot));
		}
